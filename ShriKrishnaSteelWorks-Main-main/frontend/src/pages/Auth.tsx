...
function ParticleCanvas({ isAdmin }: { isAdmin: boolean }) {
  ...
  useEffect(() => {
    ...
    const color = isAdmin ? [232, 184, 75] : [74, 144, 217];
    const COUNT = 55;
...
function VisualPanel({ portal }: { portal: Portal }) {
  const isAdmin = portal === "admin";
  return (
    <div className={`auth-visual ${isAdmin ? "auth-visual--admin" : ""}`}>
      ...
      <div className="av-orbits" aria-hidden>
        ...
        <div className={`av-dot av-dot--1 ${isAdmin ? "av-dot--admin" : ""}`} />
        <div className={`av-dot av-dot--2 ${isAdmin ? "av-dot--admin" : ""}`} />
        <div className={`av-dot av-dot--3 ${isAdmin ? "av-dot--admin" : ""}`} />
      </div>
      <div className="av-brand">
        <div className={`av-logo ${isAdmin ? "av-logo--admin" : ""}`}>
        ...
        <div className={`av-badge ${isAdmin ? "av-badge--admin" : ""}`}>
          {isAdmin ? <><span className="av-badge-dot" />Admin Console</> : <><span className="av-badge-dot" />Customer Portal</>}
        </div>
        <p className="av-tagline">
          {isAdmin ? "Secure management portal for authorised personnel" : "India's trusted industrial steel supply partner"}
        </p>
      </div>
...
export default function Auth() {
  ...
  // 1. Derive role centrally from query parameters, exactly as requested
  const urlRole = new URLSearchParams(location.search).get("role");
  const isAdmin = urlRole === "admin";
  const portal: Portal = isAdmin ? "admin" : "user";
  ...
  useEffect(() => { setErrors({}); setGlobalErr(""); }, [name, email, password, confirmPw, adminKey, location.search]);

  // Clear form when switching completely
  const clearForm = useCallback(() => {
    setName(""); setCompany(""); setEmail(""); setPassword(""); setConfirmPw("");
    setAdminKey(""); setErrors({}); setGlobalErr("");
  }, []);

  // 2. Toggling now updates the URL parameter natively
  ...
  const validate = () => {
    ...
    if (mode === "register" && isAdmin && !adminKey) e.adminKey = "Admin key required";
    setErrors(e);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === "register") {
        if (isAdmin && adminKey !== "SKW-ADMIN-2025") {
          setErrors(prev => ({ ...prev, adminKey: "Invalid admin access key" }));
          return;
        }
        await signUp(email, password, name, portal);
  ...
  // ✅ FIXED: Google login now shows toast + redirects to /home
      ...
      setGlobalErr(
        c === "auth/popup-closed-by-user" ? "Sign-in popup was closed. Try again." :
          c === "auth/popup-blocked" ? "Popup blocked. Please allow popups." :
            c === "auth/cancelled-popup-request" ? "Sign-in was cancelled." :
  ...
  return (
    <div className={`auth-page ${isAdmin ? "auth-page--admin" : ""}`}>
      ...
      <ParticleCanvas isAdmin={isAdmin} />

      <div className="auth-blobs" aria-hidden>
        <div className={`auth-blob auth-blob--1 ${isAdmin ? "auth-blob--admin" : ""}`} />
        <div className="auth-blob auth-blob--2" />
        <div className={`auth-blob auth-blob--3 ${isAdmin ? "auth-blob--admin" : ""}`} />
      </div>

      <div className="auth-layout">
        <VisualPanel portal={portal} />

        <div className="auth-panel">

          <div className={`auth-card ${flip ? "auth-card--exit" : "auth-card--enter"} ${isAdmin ? "auth-card--admin" : ""}`}>
            <div className={`auth-card-glow ${isAdmin ? "auth-card-glow--admin" : ""}`} />
            ...
            <div className="auth-portal-toggle">
              ...
              <div className={`auth-tab-pill ${isAdmin ? "auth-tab-pill--right auth-tab-pill--admin" : ""}`} />
            </div>

            <div className="auth-head">
              <div className={`auth-head-icon ${isAdmin ? "auth-head-icon--admin" : ""}`}>
                {isAdmin
              ...
              <h1 className="auth-title">
                {mode === "login" ? (isAdmin ? "Admin Sign In" : "Welcome Back") : (isAdmin ? "Admin Register" : "Create Account")}
              </h1>
              <p className="auth-subtitle">
                {mode === "login"
                  ? (isAdmin ? "Restricted — authorised personnel only" : "Sign in to your ShriKrishna account")
                  : (isAdmin ? "Register with your admin access key" : "Join and manage your steel projects")
                }
              </p>
            </div>
            ...
            <div className="auth-tabs">
              ...
              <div className={`auth-tab-pill ${mode === "register" ? "auth-tab-pill--right" : ""} ${isAdmin ? "auth-tab-pill--admin" : ""}`} />
            </div>
            ...
            {showReset ? (
              <div className="auth-reset">
                ...
                {resetSent ? (
                  ...
                  <form onSubmit={handleReset} noValidate>
                    <p className="auth-reset-info">Enter your email address and we'll send a password reset link.</p>
                    ...
                    <button type="submit" className={`auth-btn ${isAdmin ? "auth-btn--admin" : ""}`} disabled={resetBusy}>
              ...
              <form className="auth-form" onSubmit={handleSubmit} noValidate>
                ...
                {mode === "register" && !isAdmin && <AuthInput label="Company (optional)" id="a-company" placeholder="ABC Constructions Pvt. Ltd." value={company} onChange={setCompany} autoComplete="organization" icon="🏢" />}
                ...
                {mode === "register" && isAdmin && <AuthInput label="Admin Access Key" id="a-key" type="password" placeholder="Your admin key" value={adminKey} onChange={setAdminKey} error={errors.adminKey} icon="🗝" />}

                {mode === "login" && (
                  <button type="button" className="auth-forgot" onClick={() => { setShowReset(true); setResetEmail(email); }}>Forgot password?</button>
                )}

                <button type="submit" disabled={loading} className={`auth-btn ${isAdmin ? "auth-btn--admin" : ""} ${loading ? "auth-btn--loading" : ""}`}>
                  {loading ? <><span className="auth-spin" /> Processing…</> : <>{mode === "login" ? "Sign In" : "Create Account"} <span className="auth-btn-arrow">→</span></>}
                </button>
                ...
                {isAdmin && (
                  <div className="auth-secure">
                    ...
                    All admin sessions are encrypted and audited.
                  </div>
                )}
              </form>
            )}
