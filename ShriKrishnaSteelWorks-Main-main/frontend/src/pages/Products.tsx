      ...
      {customizeProduct && (
        <div className="xoverlay" onClick={() => setCustomizeProduct(null)}>
          <div className="xmodal xcmod" onClick={e => e.stopPropagation()} style={{ "--ca": customizeProduct.accentColor } as React.CSSProperties}>
            ...
            <form className="xcform" onSubmit={e => { e.preventDefault(); setShowLogin(true); }}>
              {CUSTOMIZE_FIELDS.map((f, idx) => (
                <div key={f.name} className="xcfield" style={{ animationDelay: `${idx * 0.07}s` }}>
                  ...
                  {f.type === "textarea"
                    ? <textarea className="xcinput xctarea" placeholder={f.placeholder} rows={3} value={customizeForm[f.name] || ""} onChange={e => setCustomizeForm(p => ({ ...p, [f.name]: e.target.value }))} />
                    : <input className="xcinput" type={f.type} placeholder={f.placeholder} value={customizeForm[f.name] || ""} onChange={e => setCustomizeForm(p => ({ ...p, [f.name]: e.target.value }))} />
                  }
                </div>
              ))}
              ...
              <button type="submit" className="xcsubmit" style={{ background: customizeProduct.accentColor }}>
